...
async def start_recording(roulette: Roulette, bot):
    ...
    async with async_session() as session:
        ...
        # Планируем остановку
        from services.scheduler import schedule_roulette_stop
        await schedule_roulette_stop(roulette, bot)

async def finish_roulette(roulette_id: int, bot):
    from database.engine import async_session
    async with async_session() as session:
        ...
        if not roulette or roulette.status != 'active':
            return
        # Фильтруем участников без username
        valid = []
        ...
        if roulette.winner_count > 0:
            ...
            caption = settings.get('winner_message', 'Победители: {winners}').format(
                winners=", ".join(f"@{valid[i]['username']}" for i in winners_idx)
            )
